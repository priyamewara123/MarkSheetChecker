...
export default function App() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:3000/api/verify-marksheet", formData, {
      ...
      toast.error("Failed to process marksheet. Check backend!");
    }
  };

  const RenderSection = ({ left, right }: { left: AnyObj; right: AnyObj }) => {
    ...
    return (
      <div className="flex gap-4 mt-4 justify-center">
        ...
        <div className="w-1/2 bg-white border rounded-lg shadow-sm p-3">
          <h2 className="text-sm font-semibold mb-2 border-b pb-1 text-gray-700">Scanned</h2>
              ...
              return (
                <div
                  ...
                  className={`text-xs py-1.5 px-2 rounded-md flex justify-between border ${
                    match ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"
                  }`}
                >
                  <span className="font-medium text-gray-700">{key}</span>
                  <span className={`font-semibold ${match ? "text-green-700" : "text-red-700"}`}>
                    {left[key]}
                  </span>
                </div>
              );
        ...
        <div className="w-[40px] flex flex-col items-center gap-2 pt-10">
          {filteredKeys.map((key) => {
            const match = left[key] === right[key];
            return (
              <div key={key} className={`font-bold text-xl ${match ? "text-green-500" : "text-red-500"}`}>
                ➜
              </div>
            );
          })}
        ...
        {/* RIGHT */}
        <div className="w-1/2 bg-white border rounded-lg shadow-sm p-3">
          <h2 className="text-sm font-semibold mb-2 border-b pb-1 text-gray-700">Database</h2>
          <div className="flex flex-col gap-1.5">
            {filteredKeys.map((key) => {
              const match = left[key] === right[key];
              return (
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto flex justify-center flex-col">
      <h1 className="text-xl font-bold text-center mb-4">Marksheet Verification (Diff View)</h1>
      ...
      <div className="flex justify-center mt-6">
        <div className="flex overflow-hidden bg-white border shadow rounded-lg">
          <button
            ...
            className={`px-5 py-2 text-sm font-semibold transition-all duration-300 text-gray-700 ${
          ...
          <button
            ...
            style={{ outline: "none", boxShadow: "none" }}
            className={`px-5 py-2 text-sm font-semibold transition-all duration-300 text-green-700 ${
          ...
          <button
            ...
            style={{ outline: "none", boxShadow: "none" }}
            className={`px-5 py-2 text-sm font-semibold transition-all duration-300 text-red-700 ${
      ...
      {uploadedImage && <img src={uploadedImage} className="w-full mt-4 rounded shadow-sm" alt="" />}
      ...
      {scannedData && databaseData && (
        <div className="mt-8 flex justify-center">
          <div className="w-full max-w-4xl">
            ...
            <RenderSection left={scannedData.studentInfo} right={databaseData.studentInfo} />
            ...
            <RenderSection left={scannedData.result} right={databaseData.result} />
          </div>
        </div>
      )}
    </div>
